...
#ggsave("results/cov_global_good.svg", plot = g,  units = "cm", height = 30, width = 60)
ggsave(snakemake@output[[2]], plot = g,  units = "cm", height = 30, width = 60)

# Median by Chromosome ####MAKE COLOR AES CONFIGURABLE ####

toplim <- ceiling(max(good_stats$pmedian))
values <- seq(0, toplim, by = 1)
ylabel <- "Normalized coverage"

medianplot <- ggplot(good_stats, aes(x=reorder(name, -Global_Mean, sum), y= pmedian))+
...
#ggsave("results/cov_median_good.svg", plot = medianplot,  units = "cm", height = 30, width = 60)
ggsave(snakemake@output[[3]], plot = medianplot,  units = "cm", height = 30, width = 60)

# Mean by Chromosome

toplim <- ceiling(max(good_stats$pmean))
values <- seq(0, toplim, by = 1)
ylabel <- "Normalized coverage"

meanplot <- ggplot(good_stats, aes(x=reorder(name, -Global_Mean, sum), y= pmean))+
    geom_point(aes(color= treatment))+
    ylim(0,toplim)+
    facet_grid(scale = "free_x" , space='free_x', rows= vars(Chromosome), cols = vars(group))+
    scale_color_brewer(palette = "Set2", name = "Treatment")+
