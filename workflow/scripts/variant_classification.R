...
# ref_genome_barplot_path = paste("./", ref_genome, "_variants_summary_barplot.png", sep = "")
...
samples_in_ref_genome <- metadata %>%
    group_by(ref_genome)%>%
    summarize(samples_in_ref_genome = length(unique(sample)))
...
vars_classification <- left_join(variants, effs, by = "var_id") %>%
    left_join(samples_in_ref_genome, by = "ref_genome") %>%
    left_join(presence_summary, by = "var_id") %>%
    mutate(status = ifelse(n_samples == samples_in_ref_genome, "Reference private", ifelse(n_samples == 1, "Private", "Non-private") ))%>%
    select(-c(samples_in_ref_genome, n_samples))
