...
try:
    ...
    effects = pd.concat([pd.read_csv(f, sep="\t") for f in input_effects])
    variants = pd.concat([pd.read_csv(f, sep="\t") for f in input_variants])
    lofs = pd.concat([pd.read_csv(f, sep="\t") for f in input_lofs])
    nmds = pd.concat([pd.read_csv(f, sep="\t") for f in input_nmds])
    presence = pd.concat([pd.read_csv(f, sep="\t") for f in input_presence])
